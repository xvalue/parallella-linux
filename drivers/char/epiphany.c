...
#ifdef SupportEAlloc
static int	epiphany_alloc_host_memory(epiphany_alloc_t *);
static int	epiphany_map_host_memory(struct vm_area_struct *);
#endif
...
static int epiphany_init(void)
{
	...
	if ( result < 0 ) {
		printk(KERN_INFO "epiphany_init() - Unable to add character device");
	}
	...
	*/
	...
	if ( !global_shm.kvirt_addr ) {
		printk(KERN_ERR "epiphany_init() - Unable to allocate contiguous "
...
static int epiphany_map_device_memory(struct vm_area_struct *vma)
{
	...
	if ( io_remap_pfn_range(vma, vma->vm_start, pfn, size,
							vma->vm_page_prot) ) {
		printk(KERN_ERR "epiphany_mmap - failed to map the device memory\n");
		retval = -EAGAIN;
	}
...
static int epiphany_mmap(struct file *file, struct vm_area_struct *vma)
{
	...
	printk(KERN_INFO "epiphany_mmap - request to map 0x%08lx, length 0x%08lx bytes\n",
		   off, size);
